...
export class RepairFormComponent implements OnInit {
  ...
  constructor(private _Activatedroute: ActivatedRoute, private service: HttpDataService) {
    this._Activatedroute.paramMap.subscribe(params => {
      ...
      if (this.id!=0)
      this.requestData();
      else this.repair=this.newReapair;
  ...
  create(f: any) {
    ...
    this.newReapair.property.id = f.value.propertyID;
    console.log(this.newReapair);
    ...
    if (this.responseRepair.id == -1) { this.message = "error creating repair"; }
    else {
      this.message = "Repair created!";
